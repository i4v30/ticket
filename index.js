...
client.on('ready', () => {
  ...
  client.on('messageCreate', (message) => {
    ...
    message.channel.send({embeds:[ new MessageEmbed().addFields({name:'📝 إجمالي التذاكر:',value:`${totalTickets}`},{name:`🔓 التذاكر المفتوحة:`,value:` ${openTickets}`},{name:`🔒 التذاكر المغلقة:`,value:`${closedTickets}`})]});
    }
  });
  const fs = require('fs');
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.customId === 'create_ticket') {
    ...
    if (!interaction.member.roles.cache.some(e => e.id === adminRole.id)) {
      await interaction.reply({ content: "يمكن فقط للمستخدمين الذين لديهم دور المسؤول إغلاق التذاكر.", ephemeral: true });
      return;
    }
    ...
    if (ticketData[ticketIndex].open) {
      return interaction.reply({ content: "التذكرة مفتوحة بالفعل.", ephemeral: true });
    }
    ...
    await interaction.reply({ content: "**🔓 | تم فتح التذكرة.**", ephemeral: false });  
    ...
    const adminRole = interaction.guild.roles.cache.get(adminRoleID);
    if (!interaction.member.roles.cache.some(e => e.id === adminRole.id)) {
      await interaction.reply({ content: "يمكن فقط للمستخدمين الذين لديهم دور المسؤول المطالبة بالتذاكر.", ephemeral: true });
      return;
    }
    ...
    const adminRole = interaction.guild.roles.cache.get(adminRoleID);
    if (!interaction.member.roles.cache.some(e => e.id === adminRole.id)) {
        await interaction.reply({ content: "يمكن فقط للمستخدمين الذين لديهم دور المسؤول حذف التذاكر.", ephemeral: true });
        return;
    }
